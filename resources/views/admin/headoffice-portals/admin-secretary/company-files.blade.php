...
<body>
    ...
    @include('admin.headoffice-portals.admin-secretary.partials.sidebar')
                ...
                @if (count($folders) > 0)
                    <div class="folder-grid">
                        @foreach ($folders as $rootFolder)
                            <a href="{{ route('portal.company-files.folder', [$department, $rootFolder]) }}"
                                class="folder-card">
            ...
            @if (isset($files))
                ...
                <div class="page-header">
                    ...
                    @if (!empty($parentFolder))
                        <a href="{{ route('portal.company-files.folder', [$department, $parentFolder]) }}"
                            class="back-btn">
                ...
                @if (isset($subfolders) && count($subfolders) > 0)
                    <div class="folder-grid">
                        @foreach ($subfolders as $subfolder)
                            <a href="{{ route('portal.company-files.folder', [$department, $subfolder['path']]) }}"
                                class="folder-card">
                                <div class="folder-top">
                                    <div class="folder-icon">
                                        <i class="fa-solid fa-folder"></i>
                            ...
                            <table class="files-table">
                                ...
                                <tbody>
                                    @foreach ($files as $file)
                                        @php
                                            $ext = strtolower(
                                                pathinfo($file->file_original_name ?? '', PATHINFO_EXTENSION),
                                            );
                                            $icon = 'fa-file';
