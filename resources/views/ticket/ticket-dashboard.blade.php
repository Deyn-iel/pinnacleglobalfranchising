...
<body>
                    ...
                    <div class="chat-dept-wrapper">
                        <div class="chat-dept-container">
                            <select id="chatDepartmentSelect" class="chat-dept-select">
                                ...
                                <option value="it" {{ $userDept == 'it' ? 'disabled' : '' }}>IT Tech Support
                                    Department</option>
                                <option value="hr" {{ $userDept == 'hr' ? 'disabled' : '' }}>Human Resources
                                    Department</option>
                                <option value="smm" {{ $userDept == 'smm' ? 'disabled' : '' }}>Marketing Department
                                </option>
                                <option value="admin-secretary" {{ $userDept == 'admin-secretary' ? 'disabled' : '' }}>
                                    Corp Admin Secretary</option>
                                <option value="od" {{ $userDept == 'od' ? 'disabled' : '' }}>Franchising Department
                                </option>
                                <option value="om" {{ $userDept == 'om' ? 'disabled' : '' }}>KI Operations
                                    Department</option>
                            </select>
                        </div>
                    </div>
      ...
      <div class="summary-icon"><i class="bi bi-collection"></i></div>
            ...
            <div class="controls">
                <div class="control-left w-100">
                    ...
                    <button class="btn btn-ghost filters-toggle" type="button" data-bs-toggle="collapse"
                ...
                <div class="control-right w-100">
                    <div class="tabs">
                        <div class="tab active" data-status="">All <span
                                class="count">{{ $tickets->count() }}</span></div>
                        <div class="tab" data-status="pending">Pending <span
                                class="count">{{ $tickets->where('status', 'pending')->count() }}</span></div>
                        <div class="tab" data-status="in_progress">In Progress <span
                                class="count">{{ $tickets->where('status', 'in_progress')->count() }}</span></div>
                        <div class="tab" data-status="resolved">Resolved <span
                                class="count">{{ $tickets->where('status', 'resolved')->count() }}</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-grid">
                ...
                <div class="panel">
                    <div class="panel-header">
                        <div class="hint"><i class="bi bi-info-circle me-1"></i> Click a ticket to view details
                        </div>
                        <div class="hint">
                    ...
                    <div id="ticketList">
                        @forelse($tickets as $ticket)
                            ...
                            <button type="button" class="ticket-item" data-id="{{ $ticket->id }}"
                                ...
                                <div class="ticket-card">
                                    <div class="ticket-left">
                                        ...
                                        <h6 class="ticket-title" title="{{ ucwords($ticket->department) }}">
                                        ...
                                        <div class="ticket-meta">
                                            <span class="meta-pill"><i
                                                    class="bi bi-building me-1"></i>{{ ucfirst($ticket->department) }}</span>
                                            <span class="meta-pill"><i
                                                    class="bi bi-flag me-1"></i>{{ ucfirst($ticket->priority) }}</span>
                                    ...
                                    <span class="badge-status {{ $isReview ? 'st-review' : $statusClass }}"
                                        id="status-{{ $ticket->id }}">
                        ...
                        @empty
                            <div class="empty">
                                ...
                                <button class="btn btn-black mt-2" data-bs-toggle="modal"
                ...
                <aside class="side">
                    <div class="panel">
                        ...
                        <div class="side-box">
                            <div class="side-row"><span
                                    class="k">Total</span><span>{{ $tickets->count() }}</span></div>
                            <div class="side-row"><span class="k"
                                    style="color: rgb(233, 53, 53);">Pending</span><span>{{ $tickets->where('status', 'pending')->count() }}</span>
                            </div>
                            <div class="side-row"><span class="k" style="color: #1d4ed8;">In
                                    Progress</span><span>{{ $tickets->where('status', 'in_progress')->count() }}</span>
                            </div>
                            <div class="side-row"><span class="k"
                                    style="color: #166534;">Resolved</span><span>{{ $tickets->where('status', 'resolved')->count() }}</span>
                            </div>

                            <div class="side-note">
                                Support team will respond as soon as possible. Please ensure that all ticket details are
                                complete and
                                accurate upon submission to facilitate
                                faster resolution. Kindly avoid submitting multiple requests for the same concern to
                                prevent
                                duplication and processing delays.<br><br>

                                <strong style="font-weight: bolder; color:#444444;">Reminder:</strong> Please observe
                                proper behavior and professionalism when communicating your concerns. Our support team
                                is here to assist you, and maintaining a respectful tone will help us serve you better.
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="min-w-0">
                            ...
                            <h5 class="modal-title mt-2 mb-0" id="d_department">—</h5>
                    ...
                    <form id="statusForm" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div id="cancelJustificationWrap" class="mt-3 d-none">
                                ...
                                <textarea id="cancelJustification" class="form-control mt-1" rows="3"
                            ...
                            <div id="statusSection">
                                ...
                                <div id="normalStatusWrap">
                                    ...
                                    <select name="status" id="d_statusSelect" class="form-select"
  ...
  Save
                            ...
                            <button type="button" id="submitDeclineBtn" class="btn btn-black px-4  d-none"> Save
                            </button>
